                    ...
                    <div class="chart-content">
                        <div v-if="aiData.consultationStats" class="consultation-stats">
                            <div class="stat-item">
                                ...
                                <p class="stat-value">{{ aiData.consultationStats.totalSessions }}</p>
                            </div>
                            <div class="stat-item">
                                <p class="stat-label">平均时长</p>
                                <p class="stat-value">{{ aiData.consultationStats.avgDurationMinutes }}</p>
                            </div>
                            <div class="stat-item">
                                <p class="stat-label">活跃用户</p>
                                <p class="stat-value">{{ aiData.systemOverview.activeUsers }}</p>
                            </div>
                        </div>
        ...
        <el-row style="margin-top: 20px">
            <el-card style="width: 100%">
                <template #header>
                    ...
                    <div class="chart-content">
                        <div ref="userActiveityChartRef" style="width: 100%; height: 300px;"></div>
                    </div>
            </el-card>
        </el-row>
    </div>
...
// 用户活跃度趋势
let userActiveityChart = null
const userActiveityChartRef = ref(null)
const initUserActiveityChart = () => {
    if (!userActiveityChartRef.value) return
    // 销毁现有的图表
    if (userActiveityChart) {
        userActiveityChart.dispose()
    }
    //创建echarts实例
    userActiveityChart = echarts.init(userActiveityChartRef.value)
    //获取数据
...
userActiveityChart.setOption(option)
}
