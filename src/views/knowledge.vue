<template>
    <div>
        ...
        <el-table :data="tableData" style="width: 100%; margin-top: 25px;">
            ...
            <el-table-column label="操作" width="240" fixed="right">
                <template #default="scope">
                    ...
                    <el-button @click="handlePubish(scope.row)" v-if="scope.row.status === 0 || scope.row.status === 2" text type="danger">发布</el-button>
        ...
        <el-pagination
            ...
            @change="handleChange"
        />
        <ArticleDialog v-model:modelValue="dialogVisible" :article="currentArticle" :categories="categories" @success="handleSearch" />
    </div>
</template>
<script setup>
import { onMounted,ref,reactive } from 'vue'
import PageHeads from '@/components/PageHeads.vue'
...
const formItem = [
    ...
    {comp: 'select', prop: 'category', placeholder: '请选择分类', label: '分类',},
    {comp: 'select', prop: 'status', placeholder: '请输入文章内容', label: '状态',options:[{
        label: '草稿',
        value: '0'
    },{
        label: '已发布',
